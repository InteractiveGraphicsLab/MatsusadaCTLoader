...
void t_stroke_Smooth_BSpline3Interpolation(const vector<TVector3> &CPs, vector<TVector3> &target, bool isClosed, int NumOfPoint)
{
	...
	M.createCompRowForm( isClosed ? (N * 3 ) : (N * 3 - 2) );
	
	M.solveLinearSystem_LU_prepare();
	M.solveLinearSystem_LU_solve( Px, Qx);
	M.solveLinearSystem_LU_solve( Py, Qy);
	M.solveLinearSystem_LU_solve( Pz, Qz);
...
void Tstroke3D_tube::drawTube( const float *diff, const float *ambi, const float *spec, const float* shin )const 
{
...
void Tstroke3D_fiber::deformationFiber( int grabedPointIdx, const TVector3 &point)
{
	...
	AtA.createCompRowForm(nonZero);
	AtA.solveLinearSystem_LU( Atb, result);
