...
public:   
	...
	void initAsCurvedCylinder( const vector<TVector3> &axisPs, const vector<double> &axisRad, const int rotRes, bool fillTopBottom );
	...
	void SmoothingByLaplacianZero  (int time, const TQueue<byte> &v_bFixed);
	void SmoothingByLaplacianZero  (int time, const vector<byte> &v_bFixed);
	void SmoothingByLaplacianZero_onlyMoveOnNormal(int time);
	...
	void SmoothingByLaplacianZero_fixShapePoint(int time, double dotThresh);
	void SmoothingByLaplacianZero_onlyInfrate ( int time );
	void SmoothingByLaplacianZero_onlyContract( int time );
	void SmoothingByLaplacianZero_fixDistFromOriginalPos( int time, double thresh );
	void SmoothingByLaplacianZero_cancelContraction(int time);
	...
	void extractLagestConnectRegion();
	void extractLageConnectRegion( const int minVtxNum);
	...
	void   drawPolygons_back  () const;
	void   drawPolygons_withUV() const;
	void   drawPolygons   (const bool isTransparent, const float *diff, const float *ambi, const float *spec, const float *shin) const;
	void   drawPolygons_flatShading() const;
	void   drawEdges      () const;
	void   drawEdges      (const GLfloat width, const double r, const double g, const double b) const;
	void   drawBoundEdges ();
	void   drawBoundPoints();

	//Distance
	double distanceFromPoint              ( const TVector3 &p, TVector3 &nearestPoint ) const; 
	double distanceFromPoint_searchOneRing( const TVector3 &p, TVector3 &nearestPoint ) const; 
	void  getNormalOfNearestVertex       ( const TVector3 &p, TVector3 &norm         ) const; 

	//intersection
	bool   intersectionToRay     ( const TVector3 &rayP, const TVector3 &rayD, int &polyID, TVector3 &pos, int bothFrontBack = 0 /*0both 1front 2back*/ ) const;
	bool   intersectionToRay     ( const TVector3 &rayP, const TVector3 &rayD,              TVector3 &pos, int bothFrontBack = 0 /*0both 1front 2back*/ ) const; 
	bool   intersectionToLineSegm( const TVector3 &p0  , const TVector3 &p1  , int &polyID, TVector3 &pos, double &distInP0P1) const;
	...
	void   SaveVsPsAsText_old(FILE *fp); // 旧VolMesherの実装
	bool   LoadVsPsAsText_old(FILE *fp); // 旧VolMesherの実装
	...
	inline TVector3 calcGravityCenter(){ TVector3 gc(0,0,0); for( int i=0; i<m_vSize; ++i) gc += m_verts[i]; gc /= m_vSize; return gc; }
...
protected:
	...
	bool RemoveEdgePossible1(int edgeid )const ;
...
bool t_computeInternalVoxel                ( const int W, const int H, const int D, TTriangleMesh &mesh, const TVector3 &volCubeSize,  byte *bInside );//bInsideはnewされている
