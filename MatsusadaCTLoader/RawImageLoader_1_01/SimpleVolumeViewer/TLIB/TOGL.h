...
//glewをスタティックで使いますと宣言してから glew.h を include
#ifndef GLEW_STATIC
#define GLEW_STATIC
#endif
...
class TOGL
{
	...
	bool   m_bIsDrawing ; // flg : true when 
	CPoint m_prevpos    ;
	int    m_cx, m_cy   ;
...
public:
	...
	void OnDraw_Begin_forOrtho_ScaleByEyeFocusDist(double view_near = -30, double view_far = +100)
	{
		assert(!m_bIsDrawing) ;
		m_bIsDrawing = true ;
		MakeOpenGLCurrent() ;

		double r = t_distance( m_eyeParam.m_pos, m_eyeParam.m_focus ) * 0.35;
...
inline void t_drawCubeFrame( double sizeR, float lineWidth, double red, double green, double blue)
{
	glDisable( GL_LIGHTING );
	...
	glBegin( GL_LINE_STRIP );
	glVertex3d( -sizeR, -sizeR, -sizeR); glVertex3d( +sizeR, -sizeR, -sizeR); glVertex3d( +sizeR, +sizeR, -sizeR); glVertex3d( -sizeR, +sizeR, -sizeR); glVertex3d( -sizeR, -sizeR, -sizeR);
	glVertex3d( -sizeR, -sizeR, +sizeR); glVertex3d( +sizeR, -sizeR, +sizeR); glVertex3d( +sizeR, +sizeR, +sizeR); glVertex3d( -sizeR, +sizeR, +sizeR); glVertex3d( -sizeR, -sizeR, +sizeR);
	glEnd();
	glBegin( GL_LINES );
	glVertex3d( +sizeR, -sizeR, -sizeR); glVertex3d( +sizeR, -sizeR, +sizeR);
	glVertex3d( +sizeR, +sizeR, -sizeR); glVertex3d( +sizeR, +sizeR, +sizeR);
	glVertex3d( -sizeR, +sizeR, -sizeR); glVertex3d( -sizeR, +sizeR, +sizeR); 
	glEnd();
}
...
/*-----------------------------------------
直方体のワイヤーフレームをレンダリングする
-----------------------------------------*/
inline void t_drawCuboidFrame( double sX, double sY, double sZ, float lineWidth, double red, double green, double blue)
{
	glDisable( GL_LIGHTING );
	glColor3d(  red, green,  blue);
	glLineWidth ( lineWidth );
	glBegin( GL_LINES );
		glVertex3d( 0,0,0 ); glVertex3d( sX,0,0 ); glVertex3d( sX,0,0 ); glVertex3d( sX,sY,0 ); glVertex3d( sX,sY,0 ); glVertex3d( 0,sY,0 ); glVertex3d( 0,sY,0 ); glVertex3d( 0,0,0 );
		glVertex3d( 0,0,sZ); glVertex3d( sX,0,sZ); glVertex3d( sX,0,sZ); glVertex3d( sX,sY,sZ); glVertex3d( sX,sY,sZ); glVertex3d( 0,sY,sZ); glVertex3d( 0,sY,sZ); glVertex3d( 0,0,sZ);
		glVertex3d(  0, 0, 0); glVertex3d(  0, 0,sZ); 
		glVertex3d( sX, 0, 0); glVertex3d( sX, 0,sZ);
		glVertex3d( sX, sY,0); glVertex3d( sX,sY,sZ);
		glVertex3d(  0, sY,0); glVertex3d(  0,sY,sZ);
	glEnd();
}

inline void t_drawCuboidFrame( const TVector3 &cube, float lineWidth, double red, double green, double blue)
{
	t_drawCuboidFrame( cube[0],cube[1],cube[2], lineWidth, red, green, blue);
}
