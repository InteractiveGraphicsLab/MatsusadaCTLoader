...
#pragma omp parallel for
		...
		for( int x = 0; x < W ; ++x )
		{
			...
			{
				coef += gauss55[zz+2][yy+2][xx+2];
				v    += gauss55[zz+2][yy+2][xx+2] * img[ idx + xx + yy*W + zz*WH];
			}
			tmpImg[idx] = (float) ( v / coef);
		}}
		memcpy( img, tmpImg, sizeof( float ) * WHD);
		fprintf( stderr, "smoothing done %d / %d\n", kkk, Num);
	}
	delete[] tmpImg;
}
...
double TOGL3DImage1::triLinearSampling( const double px, const double py, const double pz, const TVector3 &pos ) const
{
	return t_volLinearInterpolate<byte>( m_width, m_height, m_depth, px,py,pz, pos, m_img );  
}


double TOGL3DImage1::triLinearSampling( const TVector3 &cSize, const TVector3 &pos ) const{
	return triLinearSampling( cSize[0] / m_width ,
		                      cSize[1] / m_height,
							  cSize[2] / m_depth , pos );
}
